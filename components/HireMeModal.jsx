  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEmail('');
        setMessage('');
        // No cerramos el modal inmediatamente para mostrar el mensaje de éxito
        setTimeout(() => {
          onClose();
        }, 1500); // Cierra el modal después de 1.5 segundos
      } else {
        console.error('Error al enviar la solicitud.', data);
        setSubmissionError(data.error || 'Hubo un error al enviar tu solicitud. Por favor, intenta nuevamente.');
      }
    } catch (error) {
          ...
          <div className="modal max-w-md mx-5 xl:max-w-xl lg:max-w-xl md:max-w-xl bg-secondary-light dark:bg-primary-dark max-h-screen shadow-lg flex-row rounded-lg relative">
            <div className="modal-header flex justify-between gap-10 p-5 border-b border-ternary-light dark:border-ternary-dark">
              <h5 className=" text-primary-dark dark:text-primary-light text-xl">
                QUE TIPO DE PROYECTO QUIERES REALIZAR?
              </h5>
              <button
                onClick={onClose}
            ...
            <div className="modal-body p-5 w-full h-full">
              <form onSubmit={handleSubmit} className="max-w-xl m-4 text-left">
                ...
                <div className="mt-6 pb-4 sm:pb-1">
                  <button
                    type="submit"
                    className={`px-4 sm:px-6 py-2 sm:py-2.5 text-white bg-indigo-500 hover:bg-indigo-600 rounded-md focus:ring-1 focus:ring-indigo-900 duration-500 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                    aria-label="Submit Request"
                    disabled={isSubmitting}
                  >
                    <Button title={isSubmitting ? 'ENVIANDO...' : 'ENVIAR SOLICITUD'} />
                  </button>
                  {submissionError && (
                    <p className="mt-2 text-red-500 text-sm">{submissionError}</p>
                  )}
                  {submissionSuccess && (
                    <p className="mt-2 text-green-500 text-sm">¡Solicitud enviada con éxito!</p>
                  )}
                </div>
              </form>
            </div>
